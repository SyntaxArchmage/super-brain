...
- Ideally all would eventually (see section Inlining shape checking) be elided.
...
- Shape information that needs higher-level/graph information should use richer types (e.g.,`TensorList `);
...
The shape dialect is an IR representations and not a programming language;
...
Describe the shape inference approach that will use the shape functions;
...
Describe the approach whereby error messages will be generated;
...
Flow sensitive shape functions;
...
All static functions are usable for dynamic/unknown shapes;
