# Content from https://mlir.llvm.org/docs/Diagnostics/

Diagnostic Infrastructure - MLIR

# Diagnostic Infrastructure

This document presents an introduction to using and interfacing with MLIR’s diagnostics infrastructure.

See [MLIR specification](https://mlir.llvm.org/docs/LangRef/) for more information about MLIR, the structure of the IR, operations, etc.

## Source Locations ¶

Source location information is extremely important for any compiler, because it provides a baseline for debuggability and error-reporting. The [builtin dialect](https://mlir.llvm.org/docs/Dialects/Builtin/) provides several different location attributes types depending on the situational need.
...
Using the`DiagnosticEngine`, though, is generally not the preferred way to emit diagnostics in MLIR. [operation](https://mlir.llvm.org/docs/LangRef/#operations) provides utility methods for emitting diagnostics:
...
A`Diagnostic` in MLIR contains all of the necessary information for reporting a message to the user. A`Diagnostic` essentially boils down to four main components:
...
- Some additional information attached that can be used to identify this diagnostic other than source location and severity level (e.g. for diagnostic handlers to do some filtering). Metadata is not part of the output message.
...
Metadata is a mutable vector of DiagnosticArguments. It can be accessed and modified as a vector.
