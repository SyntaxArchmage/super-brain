...
### Filtered iterator: getOps () ¶

For example the `Block` class exposes a convenient templated method `getOps ()` that provided a filtered iterator. Here is an example:
...
The `getOps ()` is useful to iterate on some Operations immediately listed inside a single block (or a single region), however it is frequently interesting to traverse the IR in a nested fashion. To this end MLIR exposes the `walk()` helper on `Operation`, `Block`, and `Region`. This helper takes a single argument: a callback method that will be invoked for every operation recursively nested under the provided entity (as well as this initial operation).
