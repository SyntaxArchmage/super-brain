...
- `intptr_t mlirXGetNum s(MlirX)` returns the upper bound on the index.
- `MlirY mlirXGet (MlirX, intptr_t pos)` returns 'pos'-th subobject.
...
- `MlirY mlirXGetFirst (MlirX)` returns the first subobject in the list.
- `MlirY mlirYGetNextIn (MlirY)` returns the next subobject in the list that contains the given object, or a null object if the given object is the last in this list.
...
Individual interfaces are expected provide a `mlir TypeID()` function that can be used to check whether an object or a class implements this interface using `mlir ImplementsInterface` or `mlir ImplementsInterfaceStatic` functions, respectively. Rationale: C++ `isa` only works when an object exists, static methods are usually dispatched to using templates; lookup by `TypeID` in `MLIRContext` works even without an object.
