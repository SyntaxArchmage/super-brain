...
Moreover, SPIR-V supports the notion of array stride. Currently only natural strides (based on [VulkanLayoutUtils](https://github.com/llvm/llvm-project/blob/main/mlir/include/mlir/Dialect/SPIRV/LayoutUtils.h)) are supported. They are also mapped to LLVM array.

| SPIR-V Dialect | LLVM Dialect |
| --- | --- |
| `!spirv.array< x >` | `!llvm.array< x >` |
| `!spirv.rtarray< >` | `!llvm.array<0 x >` |
...
Examples of SPIR-V struct conversion are: ```mlir !spirv.struct => !llvm.struct !spirv.struct => !llvm.struct<(i8, i32)>
...
This operation is implemented as a series of LLVM Dialect operations. First step would be to create a mask with bits set outside [`offset`,`offset`+`count`- 1]. Then, unchanged bits are extracted from`base` that are outside of [`offset`,`offset`+`count`- 1]. The result is`or` ed with shifted`insert`.
...
Currently`llvm.mlir.global` s are created with`private` linkage for`Private` storage class and`External` for other storage classes, based on SPIR-V spec:
...
Note that simple conversion to`alloca` may not be sufficient if the code has some scoping. For example, if converting ops executed in a loop into`alloca` s, a stack overflow may occur. For this case,`stacksave`/`stackrestore` pair can be used (TODO).
...
This op declares or defines a SPIR-V function and it is converted to`llvm.func`. This conversion handles signature conversion, and function control attributes remapping to LLVM dialect function [passthrough attribute](https://mlir.llvm.org/docs/Dialects/LLVM/#attribute-pass-through).

The following mapping is used to map [SPIR-V function control](https://www.khronos.org/registry/spir-v/specs/unified1/SPIRV.html#_a_id_function_control_a_function_control) to [LLVM function attributes](https://llvm.org/docs/LangRef.html#function-attributes):
...
The`mlir-runner` has support for executing a`gpu` dialect kernel on the CPU via SPIR-V to LLVM dialect conversion. This is referred to as the “SPIR-V CPU Runner”. The`--link-nested-modules` flag needs to be passed for this. Currently, only single-threaded kernels are supported.
