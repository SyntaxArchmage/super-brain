...
<p>Error severity shows ops in generic form (for invalid/unstable IR); warnings/remarks use pretty custom printers. Notes must attach to a parent diagnostic via <code>attachNote</code>.</p>`,
    ...
    action: {
      name: "Action",
      role: "Tracing",
      summary: "Uniform, interceptable wrapper around any IR transformation — from a single fold to a full pass — enabling tracing, breakpoints, and debugger-style execution control.",
      definition: "An Action names and scopes one unit of compiler work at arbitrary granularity. Derived from tracing::ActionImpl<T>, it records affected IR via ArrayRef<IRUnit> (Operation, Block, or Region) and exposes a static tag for filtering. Execution is always mediated by a handler that decides whether the wrapped lambda runs, enabling debug counters (-mlir-debug-counter=tag-skip=N,tag-count=M) for bisection-style debugging without scrolling log output.",
      examples: `<pre>// Define a custom action for tiling:
class TileLoopAction : public tracing::ActionImpl<TileLoopAction> {
public:
  static constexpr StringLiteral tag = "linalg-tile-loop";
  TileLoopAction(ArrayRef<IRUnit> units, int tileSize)
      : Base(units), tileSize(tileSize) {}
  int tileSize;
};

// Dispatch through context (handler decides run/skip):
context->executeAction<TileLoopAction>(
    [&]{ /* perform tiling rewrite */ }, {loopOp}, tileSize);

// CLI bisection: skip first 5, run next 3, then stop:
// --mlir-debug-counter=linalg-tile-loop-skip=5,linalg-tile-loop-count=3</pre>
<p>Pass runs and individual pattern applications are both Action dispatch points — unifying coarse IR dumps and fine-grained tracing under one framework.</p>`,
      related: ["Pass", "RewritePattern", "Fold", "Pass Pipeline", "DiagnosticEngine"],
      usedIn: ["mlir", "compiler-ai-infra"],
      sources: [
        { label: "MLIR Action: Tracing Framework", url: "https://mlir.llvm.org/docs/ActionTracing/" }
      ]
    },
    capi: {
      name: "MLIR C API",
      role: "Infrastructure",
      summary: "Stable C interop surface exposing core MLIR IR and passes as opaque handles for non-C++ language bindings (Python, Rust, etc.).",
      definition: "The MLIR C API is the primary foreign-function interface for embedding MLIR outside C++. Core IR components (MlirOperation, MlirValue, MlirBlock, etc.) appear as opaque typedefs; manipulation happens through prefixed functions. Ownership is encoded in naming: Create/Take transfer ownership to the caller, Destroy releases it. Callers check nullity with mlirXIsNull and perform dynamic type checks with mlirXIsAY. Extension authors convert between C/C++ with wrap/unwrap helpers.",
      examples: `<pre>// Create and inspect an operation via C API:
...
if (!mlirOperationIsNull(op)) {
  ...
  // Print IR via chunked callback (never assumes null termination):
  mlirOperationPrint(op, my_append_callback, my_buffer);

  mlirOperationDestroy(op);  // caller-owned after Create
}

// Python binding built on top:
# op = Operation.create("arith.constant", results=[i32], ...)</pre>
<p>Semantic dialect-specific APIs (typed accessors) are generated by ODS on top of this flat core — the C API intentionally exposes only positional operand/attribute access.</p>`,
      related: ["Operation", "Value", "Attribute", "Dialect", "ODS", "Interface"],
      usedIn: ["mlir", "compiler-ai-infra"],
      sources: [
        { label: "MLIR C API", url: "https://mlir.llvm.org/docs/CAPI/" }
      ]
    },
    shapeFunction: {
      name: "Shape Function",
      role: "Mechanism",
      summary: "Pure, local function deriving an operation's output shape from its operands, attributes, and regions — enabling both compile-time inference and runtime validation.",
      definition: "A shape function maps operation inputs to output shape metadata along three axes: elemental type, rank, and dimensions. It must be expressible for dynamic shapes, report actionable errors, require no graph-wide dependencies, and be lowerable to inlined runtime checks. C++ implementations use InferShapedTypeOpInterface; the long-term direction is declarative shape-dialect functions. ODS traits (SameOperandsAndResultType, broadcastable) can auto-generate simple shape functions.",
      examples: `<pre>// C++ interface implementation:
LogicalResult MyOp::inferReturnTypeComponents(
  ...
  // Output shape == input shape for elementwise ops
  inferredReturnShapes.push_back(
      ShapedTypeComponents(operands[0].getType()));
  return success();
}

// ODS auto-generation via trait:
def MyOp : Op<"my.op", [SameOperandsAndResultType]> { ... }
// → shape function auto-derived: output type == operand type

// Shape dialect (declarative target):
// shape.assume_equal %lhs_shape, %rhs_shape</pre>
<p>Shape functions compose with type inference interfaces — tensors are often deducible from shaped-type inference alone, while MemRef layouts need additional handling.</p>`,
      related: ["Interface", "ODS", "Attribute", "Dialect", "Operation"],
      usedIn: ["mlir", "compiler-ai-infra"],
      sources: [
